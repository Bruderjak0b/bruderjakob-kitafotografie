...
3. **next/image in Next 16:** `priority` ist deprecated. Für das LCP-Bild `loading="eager" fetchPriority="high"` nutzen. `sizes` bei `fill`-Bildern immer setzen. Statische Imports bevorzugen (liefern Maße und Blur-Placeholder).
